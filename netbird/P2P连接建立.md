...
5. P2P 直连尝试
**客户端** 通过 internal/network/connmanager.go 尝试 与目标设备 建立 WireGuard 连接：  
按优先级（内网地址→公网地址）向 目标设备的地址 发送 WireGuard 握手包（UDP 协议）；  
若握手成功（收到目标设备的响应），则建立 P2P 连接，后续 数据 直接通过 该连接 传输（WireGuard 加密）。

6. TURN 中继兜底
若所有地址均无法直连（如对称 NAT 环境），  
客户端 向 管理服务 请求 TURN 服务器 地址，通过 TURN 中继传输数据：  
