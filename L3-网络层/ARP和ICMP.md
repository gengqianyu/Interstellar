...
```text
 0       7 8      15 16            31
|-------|-------|---------------|
| Type  | Code  |  Checksum     |   ICMP报文内容 不同类型和代码 标识不同内容
|-------|-------|---------------|
```
...
网络内：通信的源目主机在同一个广播域内，网络=广播域 PCA----PCB 内网通信不用指网关，PC 能够独立完成内网通信，Hub(集线器)/Switch(交换机) 负责网络内的数据转发。

网络间：经过 3 层网络设备 MLS,Router,FireWall 等的通信，Router 负责网络间的数据转发
...
数据帧封装，必须包含 源目主机的 MAC 地址。因此发送端必须获取到目的主机的 MAC 地址。
...
ARP 只干一件事，把为通信的 目的主机的 IP 地址映射到它的 MAC 地址，并添加到本地到 route 路由表中，过程如下：
...
> **注意**: ARP 作为一个 2 层数据链路层协议（应该是L3层协议），报文是不能穿越路由器的，不能被转发到其他广播域。
