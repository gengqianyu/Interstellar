...
| 字段 | 长度 | 说明 |
|------|------|------|
| **ID** | 16 bit | 事务标识，请求和响应使用相同的 ID 进行匹配 |
| **QR** | 1 bit | 查询/响应标志。`0` 表示查询请求，`1` 表示响应 |
| **Opcode** | 4 bit | 操作码。`0` 标准查询，`1` 反向查询，`2` 服务器状态请求 |
| **AA** | 1 bit | 授权应答（Authoritative Answer），表示响应来自授权域名服务器 |
| **TC** | 1 bit | 截断标志（Truncated），表示报文因超过传输长度而被截断 |
| **RD** | 1 bit | 期望递归（Recursion Desired），客户端设置，表示希望服务器递归查询 |
| **RA** | 1 bit | 可用递归（Recursion Available），服务器设置，表示是否支持递归查询 |
| **Z** | 3 bit | 保留字段，必须为 `0` |
| **RCODE** | 4 bit | 响应码。`0` 无差错，`1` 格式错误，`2` 服务器失败，`3` 名称不存在（NXDOMAIN） |
| **QDCOUNT** | 16 bit | 问题部分的记录数 |
| **ANCOUNT** | 16 bit | 回答部分的记录数 |
| **NSCOUNT** | 16 bit | 授权部分的记录数 |
| **ARCOUNT** | 16 bit | 附加部分的记录数 |
...
| 字段 | 说明 |
|------|------|
| **QNAME** | 要查询的域名，以标签序列编码（如 `www.github.com` 编码为 `3www6github3com0`） |
| **QTYPE** | 查询类型，如 `A`（1）、`NS`（2）、`CNAME`（5）、`MX`（15）、`AAAA`（28）等 |
| **QCLASS** | 查询类，通常为 `IN`（1），表示 Internet |

### Answer / Authority / Additional（资源记录部分）

响应报文中的回答（Answer）、授权（Authority）和附加（Additional）三个部分都使用相同的**资源记录（Resource Record, RR）** 格式：

| 字段 | 长度 | 说明 |
|------|------|------|
| **NAME** | 可变 | 资源记录对应的域名 |
| **TYPE** | 16 bit | 记录类型，如 `A`（1）、`NS`（2）、`CNAME`（5）等 |
| **CLASS** | 16 bit | 记录类，通常为 `IN`（1） |
| **TTL** | 32 bit | 生存时间（秒），表示该记录可以被缓存的时长 |
| **RDLENGTH** | 16 bit | RDATA 字段的长度（字节） |
| **RDATA** | 可变 | 资源数据，内容取决于 TYPE。例如 A 记录的 RDATA 是 4 字节的 IPv4 地址，NS 记录的 RDATA 是域名服务器的域名 |

其中三个部分的用途分别是：

- **Answer**：直接回答查询问题的资源记录（如查询的 A 记录、CNAME 记录等）
- **Authority**：指向授权域名服务器的 NS 记录，用于告知客户端哪些服务器对该域名具有权威性
- **Additional**：附加信息，通常包含 Authority 部分中 NS 记录所对应的 A 记录（即域名服务器的 IP 地址），以避免额外的查询
