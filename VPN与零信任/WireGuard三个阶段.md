...
握手的目标是协商**临时会话密钥**（避免长期使用同一密钥的安全风险），流程如下（以 A 主动连接 B 为例）：
...
- **密钥轮换**：会话密钥默认每 **120 秒**自动重新协商（通过新的握手流程），进一步提升安全性。
...
└───────────────┘     └───────────────┘     └───────────────┘     └───────────────┘
                                                                 ...
                                                                 ┌───────────────┐
                                                                 │               │     ┌───────────────┐
远程 VPN 服务器────────>│  conn 模块   │────>│  device 模块  │────>│   tun 模块    │────>│  本地应用/系统  │
                                                                 └───────────────┘     └───────────────┘
...
```text
APP -> (OS，根据系统路由规则检测出口 IP 是否为 wg 段) -> [wg tun，接收 OS 发来的包] -> [wg device，判断去向 IP 是否 allow，加密] -> [wg conn，发送给对应 IP 的 peer] -> 网卡
```

### 响应流程

```text
网卡 -> [wg conn] -> [wg device，解密，判断 IP 合法，给 tun] -> [wg tun，写入 OS] -> (OS，根据系统路由规则转发包裹到 eth0) -> APP
```
